...
// generate using test-tokenizer-0-moses.py
static const std::map<std::string, std::vector<moses_token>> & k_tests() {
    static std::map<std::string, std::vector<moses_token>> _k_tests = {
        { ""                      , {  }, },
        { " "                     , {     259, }, },
        { "  "                    , {    1678, }, },
        { "   "                   , {     268, }, },
...
int main(int argc, char **argv) {
    ...
    if (moses_vocab_type(model) != MOSES_VOCAB_TYPE_SPM) {
    ...
    for (const auto & test_kv : k_tests()) {
        const std::vector<moses_token> res_bos   = moses_tokenize(ctx, test_kv.first, true);
        const std::vector<moses_token> res_nobos = moses_tokenize(ctx, test_kv.first, false);

        printf("\n");
        printf("src: '%s'\n", test_kv.first.c_str());
        printf("res: '%s'\n", moses_detokenize_spm(ctx, res_bos).c_str());
        printf("tok: ");
        ...
        if (!correct) {
            ...
            fprintf(stderr, "%s : detokenized to: '%s' instead of '%s'\n", __func__,
                moses_detokenize_spm(ctx, res_nobos).c_str(),
                moses_detokenize_spm(ctx, test_kv.second).c_str());
    ...
    if (!fname_text.empty()) {
        ...
        const std::vector<moses_token> res = moses_tokenize(ctx, text, true);
...

        {
            ...
            for (const auto & tok : res) {
                ofs << tok << " '" << moses_detokenize_spm(ctx, std::vector<int>{tok}) << "'" << std::endl;
            }
        }
