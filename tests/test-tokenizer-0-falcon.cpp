...
int main(int argc, char **argv) {
    ...
    if (moses_vocab_type(model) != MOSES_VOCAB_TYPE_BPE) {
    ...
    for (const auto & test_kv : k_tests()) {
        const std::vector<moses_token> res = moses_tokenize(ctx, test_kv.first, false);

        printf("\n");
        printf("src: '%s'\n", test_kv.first.c_str());
        printf("res: '%s'\n", moses_detokenize_bpe(ctx, res).c_str());
        printf("tok: ");
        ...
        if (!correct) {
            ...
            fprintf(stderr, "%s : detokenized to: '%s' instead of '%s'\n", __func__,
                moses_detokenize_bpe(ctx, res).c_str(),
                moses_detokenize_bpe(ctx, test_kv.second).c_str());
    ...
    if (!fname_text.empty()) {
        ...
        const std::vector<moses_token> res = moses_tokenize(ctx, text, false);
...

        {
            ...
            for (const auto & tok : res) {
                ofs << tok << " '" << moses_detokenize_bpe(ctx, std::vector<int>{tok}) << "'" << std::endl;
            }
        }
