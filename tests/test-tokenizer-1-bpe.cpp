...
int main(int argc, char **argv) {
    ...
    GGML_ASSERT(moses_vocab_type(model) == MOSES_VOCAB_TYPE_BPE);
    ...
    for (int i = 0; i < n_vocab; ++i) {
        std::string str = moses_detokenize_bpe(ctx, std::vector<int>(1, i));
        try {
            auto cps = codepoints_from_utf8(str);
            std::vector<moses_token> tokens = moses_tokenize(ctx, str, false);
            std::string check = moses_detokenize_bpe(ctx, tokens);
            if (check != str) {
