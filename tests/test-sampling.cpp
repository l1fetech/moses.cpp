...
static void dump(const moses_token_data_array * candidates) {
...
static void test_top_k(const std::vector<float> & probs, const std::vector<float> & expected_probs, int k) {
    ...
    for (moses_token token_id = 0; token_id < (moses_token)n_vocab; token_id++) {
        const float logit = logf(probs[token_id]);
        candidates.emplace_back(moses_token_data{token_id, logit, 0.0f});
    }

    moses_token_data_array candidates_p = { candidates.data(), candidates.size(), false };
    moses_sample_softmax(nullptr, &candidates_p);
    DUMP(&candidates_p);
    moses_sample_top_k(nullptr, &candidates_p, k, 1);
    DUMP(&candidates_p);
...
static void test_top_p(const std::vector<float> & probs, const std::vector<float> & expected_probs, float p) {
    ...
    moses_sample_softmax(nullptr, &candidates_p);
    DUMP(&candidates_p);
    moses_sample_top_p(nullptr, &candidates_p, p, 1);
    DUMP(&candidates_p);
...
static void test_tfs(const std::vector<float> & probs, const std::vector<float> & expected_probs, float z) {
    ...
    moses_token_data_array candidates_p = { candidates.data(), candidates.size(), false };
    DUMP(&candidates_p);
    moses_sample_tail_free(nullptr, &candidates_p, z, 1);
    DUMP(&candidates_p);
...
static void test_min_p(const std::vector<float> & probs, const std::vector<float> & expected_probs, float p) {
    ...
    moses_token_data_array candidates_p = { candidates.data(), candidates.size(), false };
    DUMP(&candidates_p);
    moses_sample_min_p(nullptr, &candidates_p, p, 1);
    DUMP(&candidates_p);
    moses_sample_softmax(nullptr, &candidates_p);
...
static void test_typical(const std::vector<float> & probs, const std::vector<float> & expected_probs, float p) {
    ...
    moses_token_data_array candidates_p = { candidates.data(), candidates.size(), false };
    DUMP(&candidates_p);
    moses_sample_typical(nullptr, &candidates_p, p, 1);
    DUMP(&candidates_p);
...
static void test_repetition_penalties(
    const std::vector<float> & probs, const std::vector<moses_token> & last_tokens,
    ...
    moses_sample_softmax(nullptr, &candidates_p);
    DUMP(&candidates_p);
    moses_sample_repetition_penalties(nullptr, &candidates_p, (const moses_token *) last_tokens.data(), last_tokens.size(), repeat_penalty, alpha_frequency, alpha_presence);
    moses_sample_softmax(nullptr, &candidates_p);
    DUMP(&candidates_p);
...
static void test_sampler_queue(
    ...
    moses_token_data_array candidates_p = { candidates.data(), candidates.size(), false };
          ...
          moses_token min_token_id = 0;
    const moses_token max_token_id = n_vocab-1;

    for (auto s : samplers_sequence) {
        switch (s){
            case 'k': moses_sample_top_k    (nullptr, &candidates_p, top_k, 1); break;
            case 'f': GGML_ASSERT(false && "tail_free test not implemented");   break;
            case 'y': GGML_ASSERT(false && "typical test not implemented");     break;
            case 'p': moses_sample_top_p    (nullptr, &candidates_p, top_p, 1); break;
            case 'm': moses_sample_min_p    (nullptr, &candidates_p, min_p, 1); break;
            case 't': GGML_ASSERT(false && "temperature test not implemented"); break;
            default : GGML_ASSERT(false && "Unknown sampler");                  break;
        }

        moses_sample_softmax(nullptr, &candidates_p); // make sure tokens are sorted for tests

        const int size = candidates_p.size;

        if (s == 'k') {
            const int expected_size = std::min(size, top_k);
            min_token_id = std::max(min_token_id, (moses_token)(n_vocab - top_k));
            ...
            min_token_id = std::max(min_token_id, (moses_token)(n_vocab - size));
            min_token_id = std::min(min_token_id, (moses_token)(n_vocab - 1));
