#!/bin/bash
#
# Helper script for deploying moses.cpp server with a single Bash command
#
# - Works on Linux and macOS
# - Supports: CPU, CUDA, Metal, OpenCL
# - Can run all GGUF models from HuggingFace
# - Can serve requests in parallel
# - Always builds latest moses.cpp from GitHub
#
# Limitations
#
...
printf "\n"
printf "[I] This is a helper script for deploying moses.cpp's server on this machine.\n\n"
printf "    Based on the options that follow, the script might download a model file\n"
printf "    from the internet, which can be a few GBs in size. The script will also\n"
printf "    build the latest moses.cpp source code from GitHub, which can be unstable.\n"
printf "\n"
printf "    Upon success, an HTTP server will be started and it will serve the selected\n"
printf "    model using moses.cpp for demonstration purposes.\n"
printf "\n"
printf "    Please note:\n"
printf "\n"
...
printf "    - Do not use this script to benchmark moses.cpp\n"
...
# get latest moses.cpp and build

printf "[+] Downloading latest moses.cpp\n"

moses_cpp_dir="__moses_cpp_port_${port}__"

if [[ -d "$moses_cpp_dir" && ! -f "$moses_cpp_dir/__ggml_script__" ]]; then
    # if the dir exists and there isn't a file "__ggml_script__" in it, abort
    printf "[-] Directory %s already exists\n" "$moses_cpp_dir"
    printf "[-] Please remove it and try again\n"
    exit 1
elif [[ -d "$moses_cpp_dir" ]]; then
    printf "[+] Directory %s already exists\n" "$moses_cpp_dir"
    printf "[+] Using cached moses.cpp\n"
    ...
    printf "[+] Cloning moses.cpp\n"

    git clone https://github.com/l1fetech/moses.cpp "$moses_cpp_dir"
fi

# mark that that the directory is made by this script
touch "$moses_cpp_dir/__ggml_script__"
...
if [[ "$backend" == "cuda" ]]; then
    ...
    MOSES_CUBLAS=1 make -j server $log
    ...
    MOSES_CLBLAST=1 make -j server $log
else
