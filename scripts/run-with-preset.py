...
description = """Run moses.cpp binaries with presets from YAML file(s).
To specify which binary should be run, specify the "binary" property (main, perplexity, moses-bench, and server are supported).
To get a preset file template, run a moses.cpp binary with the "--logdir" CLI argument.

Formatting considerations:
- The YAML property names are the same as the CLI argument names of the corresponding binary.
- Properties must use the long name of their corresponding moses.cpp CLI arguments.
- Like the moses.cpp binaries the property names do not differentiate between hyphens and underscores.
...
elif binary.lower().endswith("moses-bench"):
    cli_args = CLI_ARGS_MOSES_BENCH
