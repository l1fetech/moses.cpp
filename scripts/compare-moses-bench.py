...
DESCRIPTION = """Creates tables from moses-bench data written to an SQLite database. Example usage (Linux):

$ git checkout master
$ make clean && make moses-bench
$ ./moses-bench -o sql | sqlite3 moses-bench.sqlite
$ git checkout some_branch
$ make clean && make moses-bench
$ ./moses-bench -o sql | sqlite3 moses-bench.sqlite
$ ./scripts/compare-moses-bench.py

Performance numbers from multiple runs per commit are averaged WITHOUT being weighted by the --repetitions parameter of moses-bench.
"""
...
help_c = (
    ...
    "Accepts either a branch name, tag name, or commit hash. "
    "Defaults to the non-master commit for which moses-bench was run most recently."
)
parser.add_argument("-c", "--compare", help=help_c)
help_i = (
    "Input SQLite file for comparing commits. "
    "Defaults to 'moses-bench.sqlite' in the current working directory. "
...
help_s = (
    ...
    "specified values are averaged WITHOUT weighing by the --repetitions parameter of moses-bench."
)
...
if input_file is None and os.path.exists("./moses-bench.sqlite"):
    input_file = "moses-bench.sqlite"
...
if known_args.compare is not None:
    ...
    if hexsha8_compare is None:
        ...
        sys.exit(1)
# Otherwise, search for the commit for moses-bench was most recently run
