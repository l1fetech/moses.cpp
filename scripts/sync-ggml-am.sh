#!/bin/bash
#
# Synchronize ggml changes to moses.cpp
#
# Usage:
#
#   $ cd /path/to/moses.cpp
...
lc=$(cat $SRC_MOSES/scripts/sync-ggml.last)
...
git log --oneline $lc..HEAD --reverse | grep -v "(moses/[0-9]*)" | cut -d' ' -f1 > $SRC_MOSES/ggml-commits

if [ ! -s $SRC_MOSES/ggml-commits ]; then
    rm -v $SRC_MOSES/ggml-commits
    echo "No new commits"
    exit 0
fi

if [ -f $SRC_MOSES/ggml-src.patch ]; then
    rm -v $SRC_MOSES/ggml-src.patch
fi
...
while read c; do
    ...
    git format-patch -k $c~1..$c --stdout -- \
        ...
        >> $SRC_MOSES/ggml-src.patch
done < $SRC_MOSES/ggml-commits

rm -v $SRC_MOSES/ggml-commits

# delete files if empty
if [ ! -s $SRC_MOSES/ggml-src.patch ]; then
    rm -v $SRC_MOSES/ggml-src.patch
fi
...
# update last commit
cd $SRC_GGML
git log -1 --format=%H > $SRC_MOSES/scripts/sync-ggml.last
