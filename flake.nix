# The flake interface to moses.cpp's Nix expressions. The flake is used as a
...
# want to consider consuming the overlay, or instantiating `mosesPackages`
# directly:
#
# ```nix
# pkgs.callPackage ${moses-cpp-root}/.devops/nix/scope.nix { }`
# ```
...
# of the relation between Nix and the Nix Flakes.
{
  ...
  # nixConfig = {
  #   extra-substituters = [
  #     # Populated by the CI in ggerganov/moses.cpp
  #     "https://moses-cpp.cachix.org"
  #
  ...
  #   # - https://app.cachix.org/cache/moses-cpp
  #   # - https://app.cachix.org/cache/cuda-maintainers
  #   extra-trusted-public-keys = [
  #     "moses-cpp.cachix.org-1:H75X+w83wUKTIPSO1KWy9ADUrzThyGs8P5tmAbkWhQc="
  #     "cuda-maintainers.cachix.org-1:0dq3bujKpuEPMCX6U4WylrUDZ9JyUG0VpVZa7CNfq5E="
  #   ];
  # };
  # ```

  # For inspection, use `nix flake show github:ggerganov/moses.cpp` or the nix repl:
  #
  # ```bash
  # ❯ nix repl
  # nix-repl> :lf github:ggerganov/moses.cpp
  # Added 13 variables.
  # nix-repl> outputs.apps.x86_64-linux.quantize
  # { program = "/nix/store/00000000000000000000000000000000-moses.cpp/bin/quantize"; type = "app"; }
  # ```
  outputs =
    ...
    let
      ...
      # mosesVersion = self.dirtyShortRev or self.shortRev;
    ...
    flake-parts.lib.mkFlake { inherit inputs; }

      {
        ...
        # An overlay can be used to have a more granular control over moses-cpp's
        ...
        #     overlays = [ (moses-cpp.overlays.default) ];
        ...
        # }; in {
        #     packages.aarch64-linux.mosesJetsonXavier = pkgs.mosesPackages.moses-cpp;
        # }
        # ```
        #
        ...
        flake.overlays.default =
          (final: prev: {
            mosesPackages = final.callPackage .devops/nix/scope.nix { inherit mosesVersion; };
            inherit (final.mosesPackages) moses-cpp;
          });
        ...
        perSystem =
          ...
          }:
          {
            ...
            # access them as `nix build .#mosesPackages.${scriptName}` using
            # the same path you would with an overlay.
            legacyPackages = {
              mosesPackages = pkgs.callPackage .devops/nix/scope.nix { inherit mosesVersion; };
              mosesPackagesCuda = pkgsCuda.callPackage .devops/nix/scope.nix { inherit mosesVersion; };
              mosesPackagesRocm = pkgsRocm.callPackage .devops/nix/scope.nix { inherit mosesVersion; };
            };
            ...
            packages =
              {
                default = config.legacyPackages.mosesPackages.moses-cpp;
              }
              // lib.optionalAttrs pkgs.stdenv.isLinux {
                opencl = config.packages.default.override { useOpenCL = true; };
                cuda = config.legacyPackages.mosesPackagesCuda.moses-cpp;

                mpi-cpu = config.packages.default.override { useMpi = true; };
                mpi-cuda = config.packages.default.override { useMpi = true; };
              }
              // lib.optionalAttrs (system == "x86_64-linux") {
                rocm = config.legacyPackages.mosesPackagesRocm.moses-cpp;
              };
