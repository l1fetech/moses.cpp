...
struct train_state  * init_train_state() {
    ...
    state->opt->params.graph_size = MOSES_TRAIN_MAX_NODES;
...
int64_t get_example_targets_batch(
    ...
    moses_token bos = moses_token_bos(moses_get_model(lctx));
    moses_token eos = moses_token_eos(moses_get_model(lctx));
    ...
    for (int k=0; k<n_batch; ++k) {
        ...
        for (int64_t i=0; i<n_tokens; ++i) {
            ...
            if (sample_offs < sample_size) {
                token = clamp(train_data[sample_begin+sample_offs], 0, (moses_token) (n_vocab - 1));
                ++sample_offs;
            }
...
size_t tokenize_file(
        ...
        std::vector<moses_token> & out_tokens,
        std::vector<size_t>      & out_samples_begin,
        std::vector<size_t>      & out_samples_size) {
    struct moses_file f(filename, "rb");
    ...
    // e.g. '\t' will be tokenized by moses spm tokenizer to [29871, 12]
    ...
    if (sample_start.size() == 0) {
        ...
        int n_tokens = moses_tokenize(
            moses_get_model(lctx),
            buf.data(),
            (int) buf.size(),
            out_tokens.data(),
            (int) out_tokens.size(),
        ...
        std::vector<moses_token> tok_sample;
        ...
        int n_vocab = moses_n_vocab(moses_get_model(lctx));
        for (moses_token token=0; token < n_vocab; ++token) {
            max_token_text_size = std::max(
                max_token_text_size,
                strlen(moses_token_get_text(moses_get_model(lctx), token)));
        }
        ...
        for (unsigned i=0; i<out_samples_begin.size(); ++i) {
            ...
            if (sample_size > 0) {
                // moses_tokenize expects zero terminated string,
                ...
                int n_tokens = moses_tokenize(moses_get_model(lctx),
                    buf_sample.data(),
                    (int) buf_sample.size(),
                    tok_sample.data(),
                    (int) tok_sample.size(),
                ...
                if (n_tokens < 0) {
                    ...
                    n_tokens = moses_tokenize(moses_get_model(lctx),
                        buf_sample.data(),
                        (int) buf_sample.size(),
                        tok_sample.data(),
                        (int) tok_sample.size(),
