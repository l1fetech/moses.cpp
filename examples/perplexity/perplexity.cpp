...
static results_perplexity perplexity_v2(moses_context * ctx, const gpt_params & params) {
    // Download: https://s3.amazonaws.com/research.metamind.io/wikitext/wikitext-2-raw-v1.zip?ref=salesforce-research
    // Run `./perplexity -m models/7B/ggml-model-q4_0.bin -f wiki.test.raw`
    // Output: `perplexity: 13.5106 [114/114]`
    // BOS tokens will be added for each chunk before eval

    const bool add_bos = moses_should_add_bos_token(moses_get_model(ctx));

    fprintf(stderr, "%s: tokenizing the input ..\n", __func__);

    std::vector<moses_token> tokens = ::moses_tokenize(ctx, params.prompt, add_bos);
    ...
    const int n_vocab = moses_n_vocab(moses_get_model(ctx));
    ...
    for (int i = 0; i < n_chunk; ++i) {
        ...
        for (int j = 0; j < num_batches; ++j) {
            ...
            if (moses_decode(ctx, moses_batch_get_one(tokens.data() + batch_start, batch_size, j * n_batch, 0))) {
            ...
            if (add_bos && j == 0) {
                tokens[batch_start] = moses_token_bos(moses_get_model(ctx));
            }

            const auto batch_logits = moses_get_logits(ctx);
...
static results_perplexity perplexity(moses_context * ctx, const gpt_params & params) {
    if (params.ppl_stride > 0) {
    ...
    for (int i = 0; i < n_chunk; ++i) {
        ...
        // from moses_eval above.  Now, based on https://huggingface.co/docs/transformers/perplexity,
        ...
        const float * all_logits = num_batches > 1 ? logits.data() : moses_get_logits(ctx);
        if (!params.logits_file.empty()) {
...
static bool decode_helper(moses_context * ctx, moses_batch & batch, std::vector<float> & batch_logits, int32_t n_batch, int32_t n_vocab) {
    for (int32_t i = 0; i < (int32_t) batch.n_tokens; i += n_batch) {
        ...
        memcpy(batch_logits.data() + i*n_vocab, moses_get_logits(ctx), n_tokens*n_vocab*sizeof(float));
    }
...
static void compute_logprobs(const float * batch_logits, int n_vocab, std::vector<std::thread>& workers,
        const std::vector<std::pair<size_t, moses_token>>& eval_pairs, std::vector<float>& eval_results) {
...
static void hellaswag_score(moses_context * ctx, const gpt_params & params) {
    ...
    const bool is_spm = moses_vocab_type(moses_get_model(ctx)) == MOSES_VOCAB_TYPE_SPM;
    ...
    for (size_t i = 0; i < hs_task_count; i++) {
        ...
        for (size_t j = 0; j < 4; j++) {
            ...
            hs_cur.seq_tokens[j] = ::moses_tokenize(ctx, hs_cur.context + " " + hs_cur.ending[j], add_bos);
        }
        ...
        //GGML_ASSERT(hs_cur.common_prefix >= ::moses_tokenize(ctx, hs_cur.context, add_bos).size());
    ...
    moses_batch batch = moses_batch_init(n_ctx, 0, max_seq);

    std::vector<float> tok_logits(n_vocab);
    std::vector<float> batch_logits(n_vocab*n_ctx);

    std::vector<std::pair<size_t, moses_token>> eval_pairs;
    ...
    for (size_t i0 = 0; i0 < hs_task_count; i0++) {
        ...
        size_t i1 = i0;
        size_t i_batch = 0; // this tells us where in `moses_batch` we are currently
        ...
        while (n_cur + (int) hs_data[i1].required_tokens <= n_ctx) {
            ...
            for (size_t i = 0; i < hs_cur.common_prefix; ++i) {
                moses_batch_add(batch, hs_cur.seq_tokens[0][i], i, { s0 + 0, s0 + 1, s0 + 2, s0 + 3}, false);
            }
            batch.logits[batch.n_tokens - 1] = true; // we need logits for the last token of the common prefix

            for (int s = 0; s < 4; ++s) {
                for (size_t i = hs_cur.common_prefix; i < hs_cur.seq_tokens[s].size(); ++i) {
                    moses_batch_add(batch, hs_cur.seq_tokens[s][i], i, { s0 + s }, true);
                }
            }
        ...
        if (!decode_helper(ctx, batch, batch_logits, n_batch, n_vocab)) {
            fprintf(stderr, "%s: moses_decode() failed\n", __func__);
            return;
        }
...
struct winogrande_entry {
    ...
    std::vector<moses_token> seq_tokens[2];
};
 ...
 * You can get one such dataset from e.g. https://huggingface.co/datasets/ikawrakow/winogrande-eval-for-moses.cpp
 ...
 *
 */
static void winogrande_score(moses_context * ctx, const gpt_params & params) {
    ...
    for (auto & task : data) {
        task.seq_tokens[0] = ::moses_tokenize(ctx, task.first + task.choices[0] + task.second, add_bos);
        task.seq_tokens[1] = ::moses_tokenize(ctx, task.first + task.choices[1] + task.second, add_bos);
        ...
        task.n_base1 = ::moses_tokenize(ctx, task.first + task.choices[0], add_bos).size();
        task.n_base2 = ::moses_tokenize(ctx, task.first + task.choices[1], add_bos).size();
    }
    ...
    for (size_t i0 = 0; i0 < data.size(); i0++) {
        ...
        while (n_cur + (int) data[i1].required_tokens <= n_ctx) {
            ...
            for (size_t i = 0; i < data[i1].common_prefix; ++i) {
                moses_batch_add(batch, data[i1].seq_tokens[0][i], i, { s0 + 0, s0 + 1}, false);
            }
            batch.logits[batch.n_tokens - 1] = true;

            for (int s = 0; s < 2; ++s) {
                for (size_t i = data[i1].common_prefix; i < data[i1].seq_tokens[s].size(); ++i) {
                    moses_batch_add(batch, data[i1].seq_tokens[s][i], i, { s0 + s }, true);
                }
            }
...
struct multiple_choice_task {
    ...
    std::vector<std::vector<moses_token>> seq_tokens;
    std::vector<float> log_probs;
};

static bool multiple_choice_prepare_one_task(moses_context * ctx, bool add_bos, multiple_choice_task& task, bool log_error) {
    ...
    for (auto& answer : task.mc1.answers) {
        ...
        task.seq_tokens.emplace_back(::moses_tokenize(ctx, task.question + " " + answer, add_bos));
    }
...
//     https://huggingface.co/datasets/ikawrakow/validation-datasets-for-moses.cpp
...
//
static void multiple_choice_score(moses_context * ctx, const gpt_params & params) {
    ...
    for (size_t i0 = 0; i0 < tasks.size(); i0++) {
        ...
        while (n_cur + (int) tasks[i1].required_tokens <= n_ctx) {
            ...
            for (size_t i = 0; i < cur_task.common_prefix; ++i) {
                //moses_batch_add(batch, cur_task.seq_tokens[0][i], i, { s0 + 0, s0 + 1, s0 + 2, s0 + 3}, false);
                moses_batch_add(batch, cur_task.seq_tokens[0][i], i, batch_indeces, false);
            }
            batch.logits[batch.n_tokens - 1] = true; // we need logits for the last token of the common prefix

            for (int s = 0; s < int(cur_task.seq_tokens.size()); ++s) {
                for (size_t i = cur_task.common_prefix; i < cur_task.seq_tokens[s].size(); ++i) {
                    moses_batch_add(batch, cur_task.seq_tokens[s][i], i, { s0 + s }, true);
                }
            }
...
static void kl_divergence(moses_context * ctx, const gpt_params & params) {
    if (params.logits_file.empty()) {
    ...
    if (n_vocab != moses_n_vocab(moses_get_model(ctx))) {
        fprintf(stderr, "%s: inconsistent vocabulary (%d vs %d)\n", __func__, n_vocab, moses_n_vocab(moses_get_model(ctx)));
    }

    std::vector<moses_token> tokens(n_ctx * n_chunk);
