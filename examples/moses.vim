" Requires an already running moses.cpp server
" To install either copy or symlink to ~/.vim/autoload/moses.vim
" Then start with either :call moses#doMosesGen(),
" or add a keybind to your vimrc such as
" nnoremap Z :call moses#doMosesGen()<CR>
" Similarly, you could add an insert mode keybind with
" inoremap <C-B> <Cmd>call moses#doMosesGen()<CR>
"
" g:moses_api_url, g:moses_api_key and g:moses_overrides can be configured in your .vimrc
" let g:moses_api_url = "192.168.1.10:8080"
" moses_overrides can also be set through buffer/window scopes. For instance
" autocmd filetype python let b:moses_overrides = {"temp": 0.2}
...
" server default -> g:moses_overrides -> w:moses_overrides ->
" b:moses_overrides -> in file (!*) overrides
"
...
if !exists("g:moses_api_url")
    let g:moses_api_url= "127.0.0.1:8080"
endif
if !exists("g:moses_overrides")
   let g:moses_overrides = {}
endif
const s:querydata = {"n_predict": 256, "stop": [ "\n" ], "stream": v:true }
const s:curlcommand = ['curl','--data-raw', "{\"prompt\":\"### System:\"}", '--silent', '--no-buffer', '--request', 'POST', '--url', g:moses_api_url .. '/completion', '--header', "Content-Type: application/json"]
let s:linedict = {}
...
func moses#doMosesGen()
   ...
   call extend(l:querydata, g:moses_overrides)
   if exists("w:moses_overrides")
      call extend(l:querydata, w:moses_overrides)
   endif
   if exists("b:moses_overrides")
      call extend(l:querydata, b:moses_overrides)
   endif
   if l:buflines[0][0:1] == '!*'
   ...
   if exists("g:moses_api_key")
       call extend(l:curlcommand, ['--header', 'Authorization: Bearer ' .. g:moses_api_key])
   endif
...
func moses#tokenizeWord(...)
    ...
    let l:curlcommand = copy(s:curlcommand)
    let l:curlcommand[2] = json_encode(l:querydata)
    let l:curlcommand[8] = g:moses_api_url .. "/tokenize"
...
" Example usage :echo moses#tokenCount()
func moses#tokenCount(...)
    if (a:0 > 0)
        let l:buflines = a:1
    else
