...
struct my_moses_hparams {
    ...
    float f_norm_rms_eps = 1e-5f; // moses
    ...
    bool operator!=(const my_moses_hparams& other) const {
        return memcmp(this, &other, sizeof(other));
    }
};

struct my_moses_layer {
    // normalization
...
struct my_moses_model {
    struct my_moses_hparams hparams;
    ...
    std::vector<my_moses_layer> layers;
};

struct my_moses_lora_hparams {
    uint32_t lora_r = 1;
    ...
    bool operator!=(const my_moses_lora_hparams& other) const {
        return memcmp(this, &other, sizeof(other));
    }
};

struct my_moses_lora_layer {
    // normalization
...
struct my_moses_lora {
    struct ggml_context * ctx = NULL;
    std::vector<uint8_t> data;

    my_moses_lora_hparams hparams;
    ...
    std::vector<my_moses_lora_layer> layers;
};
...
static const char * LLM_KV_ROPE_FREQ_BASE              = "%s.rope.freq_base"; // TODO load in moses.cpp
...
static void print_params(struct my_moses_hparams * params) {
...
static void print_lora_params(struct my_moses_lora_hparams * params) {
...
static void load_model_hparams_gguf(struct gguf_context * ctx, struct my_moses_hparams * hparams, const char * expected_arch) {
    std::string arch;
...
static void init_model(struct moses_model * input, struct my_moses_model * model, const char * fn_model, uint32_t n_ctx) {
    ...
    // get parameters directly from gguf file
    {
        ...
        load_model_hparams_gguf(mctx, &hparams, "moses");

        gguf_free(mctx);
    }
    hparams.n_vocab = moses_n_vocab(input);
    hparams.n_ctx = n_ctx;

    // get tensors from moses_model (possibly mmapped)
    model->tok_embeddings = moses_get_model_tensor(input, tn(LLM_TENSOR_TOKEN_EMBD));
    model->norm           = moses_get_model_tensor(input, tn(LLM_TENSOR_OUTPUT_NORM));
    model->output         = moses_get_model_tensor(input, tn(LLM_TENSOR_OUTPUT));
    ...
    for (uint32_t i = 0; i < hparams.n_layer; ++i) {
        ...
        layer.attention_norm = moses_get_model_tensor(input, tni(LLM_TENSOR_ATTN_NORM, i));
        layer.wq             = moses_get_model_tensor(input, tni(LLM_TENSOR_ATTN_Q, i));
        layer.wk             = moses_get_model_tensor(input, tni(LLM_TENSOR_ATTN_K, i));
        layer.wv             = moses_get_model_tensor(input, tni(LLM_TENSOR_ATTN_V, i));
        layer.wo             = moses_get_model_tensor(input, tni(LLM_TENSOR_ATTN_OUT, i));
        layer.ffn_norm       = moses_get_model_tensor(input, tni(LLM_TENSOR_FFN_NORM, i));
        layer.w1             = moses_get_model_tensor(input, tni(LLM_TENSOR_FFN_GATE, i));
        layer.w2             = moses_get_model_tensor(input, tni(LLM_TENSOR_FFN_DOWN, i));
        layer.w3             = moses_get_model_tensor(input, tni(LLM_TENSOR_FFN_UP, i));
...
static void set_param_lora(struct my_moses_lora * lora) {
...
static void alloc_lora(struct ggml_allocr * alloc, struct my_moses_lora * lora) {
...
static void init_lora(const struct my_moses_model * model, struct my_moses_lora * lora) {
...
static void randomize_lora(struct my_moses_lora * lora, int seed, float mean, float std, float min, float max) {
    const uint32_t n_layer = lora->layers.size();
...
static struct ggml_tensor * moses_build_lora_finetune_graphs(
        struct my_moses_model * model,
        struct my_moses_lora  * lora,
    ...
    for (int il = 0; il < n_layer; ++il) {
        struct my_moses_layer & layer = model->layers[il];
        struct my_moses_lora_layer & llayer = lora->layers[il];
...
static void load_moses_lora_gguf(struct gguf_context * fctx, struct ggml_context * f_ggml_ctx, struct my_moses_model * model, struct my_moses_lora * lora) {
...
static void save_moses_lora_gguf(struct gguf_context * fctx, struct my_moses_model * model, struct my_moses_lora * lora) {
    const char * arch = "moses";
    enum moses_ftype ftype = MOSES_FTYPE_ALL_F32;
...
static void load_checkpoint_lora_gguf(struct gguf_context * fctx, struct ggml_context * f_ggml_ctx, struct my_moses_model * model, struct my_moses_lora * lora, struct train_state * train) {
    ...
    load_moses_lora_gguf(fctx, f_ggml_ctx, model, lora);
}

static void save_checkpoint_lora_gguf(struct gguf_context * fctx, struct my_moses_model * model, struct my_moses_lora * lora, struct train_state * train) {
    gguf_set_val_str(fctx, LLM_KV_TRAINING_TYPE, LLM_KV_TRAINING_TYPE_FINETUNE_LORA);
    save_moses_lora_gguf(fctx, model, lora);
    save_train_state_gguf(fctx, train);
}

static bool load_checkpoint_lora_file(const char * filename, struct my_moses_model * model, struct my_moses_lora * lora, struct train_state * train) {
...
static void save_checkpoint_lora_file(const char * filename, struct my_moses_model * model, struct my_moses_lora * lora, struct train_state * train) {
...
static void write_tensor(struct moses_file * file, struct ggml_tensor * tensor, const char * name) {
    if (tensor == NULL) {
        file->write_u32(0);
        file->write_u32(0);
...
static void save_as_moses_lora(const char * filename, struct my_moses_lora * lora) {
    printf("%s: saving to %s\n", __func__, filename);
    struct moses_file file(filename, "wb");
    if (file.fp == NULL) {
        return;
    }
    ...
    // write_magic
    file.write_u32(MOSES_FILE_MAGIC_GGLA);   // magic
...
static void train_print_usage(int argc, char ** argv, const struct train_params * params) {
    ...
    fprintf(stderr, "  --lora-out FNAME           path to save moses lora (default '%s')\n", params->fn_lora_out);
    fprintf(stderr, "  --only-write-lora          only save moses lora, don't do any training.  use this if you only want to convert a checkpoint to a lora adapter.\n");
...
static void save_train_files(void * vdata, struct train_state * train) {
    ...
    if (strlen(data->fn_lora_out) > 0) {
        save_as_moses_lora(get_train_filename(data->fn_lora_out, data->pattern_fn_it, data->fn_latest, iter).c_str(), data->lora);
        save_as_moses_lora(get_train_filename(data->fn_lora_out, data->pattern_fn_it, data->fn_latest, -1  ).c_str(), data->lora);
    }
}

static int64_t get_parameter_count(struct my_moses_lora* lora) {
    int64_t nx = 0;
...
int main(int argc, char ** argv) {
    ...
    if (params.common.seed == MOSES_DEFAULT_SEED) {
        params.common.seed = time(NULL);
    }
    printf("%s: seed: %u\n", __func__, params.common.seed);
    srand(params.common.seed);

    struct moses_model_params moses_mparams = moses_model_default_params();
    moses_mparams.n_gpu_layers = params.common.n_gpu_layers;
    moses_mparams.vocab_only = false;

    printf("%s: model base = '%s'\n", __func__, params.fn_model_base);
    struct moses_model * lmodel = moses_load_model_from_file(params.fn_model_base, moses_mparams);

    struct moses_context_params moses_cparams = moses_context_default_params();
    struct moses_context * lctx = moses_new_context_with_model(lmodel, moses_cparams);
    ...
    opt->params.graph_size              = MOSES_TRAIN_MAX_NODES;
    ...
    const size_t estimated_compute_size_wo_data = (
            2*MOSES_TRAIN_MAX_NODES*ggml_tensor_overhead() +
            (params.common.use_checkpointing ? 3 : 2)*(GGML_OBJECT_SIZE+ggml_graph_overhead_custom(MOSES_TRAIN_MAX_NODES, true))
    );
    ...
    for (unsigned order = 0; order < (unsigned) GGML_CGRAPH_EVAL_ORDER_COUNT; ++order) {
        ...
        gf = ggml_new_graph_custom(ctx_compute, MOSES_TRAIN_MAX_NODES, true);
        gf->order = (enum ggml_cgraph_eval_order) order;
        gb = ggml_new_graph_custom(ctx_compute, MOSES_TRAIN_MAX_NODES, true);
        gb_tmp = params.common.use_checkpointing
            ? ggml_new_graph_custom(ctx_compute, MOSES_TRAIN_MAX_NODES, true)
            : NULL;
        loss = moses_build_lora_finetune_graphs(
    ...
    // tokenize data
    std::vector<moses_token> train_tokens;
