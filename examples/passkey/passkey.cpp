...
int main(int argc, char ** argv) {
    ...
    ctx_params.n_ctx   = moses_n_ctx_train(model)*n_grp + n_keep;
    ...
    // tokenize the prompt
    std::vector<moses_token> tokens_list;
    tokens_list = ::moses_tokenize(ctx, params.prompt, true);

    // tokenize the prefix and use it as a sink
    const int n_tokens_prefix = ::moses_tokenize(ctx, prompt_prefix, true).size();
    ...
    const int n_ctx       = moses_n_ctx(ctx) - n_keep;
    const int n_kv_req    = moses_n_ctx(ctx);
    ...
    moses_batch batch = moses_batch_init(512, 0, 1);
    ...
    for (int i = 0; i < n_ctx; i += n_batch) {
        if (i > 0 && n_grp > 1) {
            ...
            moses_kv_cache_seq_shift(ctx, 0, n_past - n_batch,         n_past,         ib*bd);
            moses_kv_cache_seq_div  (ctx, 0, n_past - n_batch + ib*bd, n_past + ib*bd, n_grp);
        ...
        for (int j = 0; j < n_batch && i + j < n_tokens_all; j++) {
            moses_batch_add(batch, tokens_list[i + j], n_past++, { 0 }, false);
        }
    ...
    for (int i = n_ctx; i < n_tokens_all; i += n_batch) {
        ...
        moses_kv_cache_seq_rm   (ctx, 0, n_keep            , n_keep + n_discard);
        moses_kv_cache_seq_shift(ctx, 0, n_keep + n_discard, n_ctx,  -n_discard);
    ...
    while (n_cur <= n_len) {
        // sample the next token
        {
            ...
            auto * logits  = moses_get_logits_ith(ctx, batch.n_tokens - 1);
            ...
            LOG_TEE("%s", moses_token_to_piece(ctx, new_token_id).c_str());
            fflush(stdout);
            ...
            moses_batch_add(batch, new_token_id, n_past++, { 0 }, true);
        }

        n_cur += 1;

        // evaluate the current batch with the transformer model
        if (moses_decode(ctx, batch)) {
