...
#ifdef MOSES_DEFAULT_RMS_EPS
constexpr float rms_norm_eps = MOSES_DEFAULT_RMS_EPS;
#else
...
struct moses_hparams {
    ...
    bool operator!=(const moses_hparams & other) const {
        return memcmp(this, &other, sizeof(moses_hparams));
    }
};

static uint32_t get_n_ff(const struct moses_hparams* hparams) {
    const uint32_t n_ff = ((2*(4*hparams->n_embd)/3 + hparams->n_mult - 1)/hparams->n_mult)*hparams->n_mult;
    return n_ff;
}

struct moses_hparams_lora {
    uint32_t n_vocab = 32000;
    uint32_t n_ctx   = 512;   // this is provided as user input?
    uint32_t n_embd  = 4096;
    uint32_t n_mult  = 4;
    ...
    bool operator!=(const moses_hparams_lora & other) const {
        return memcmp(this, &other, sizeof(moses_hparams_lora)) != 0;
    }
};
...
struct moses_model_lora {
    struct ggml_context * ctx = NULL;

    moses_hparams_lora hparams;
    ...
    std::vector<moses_layer_lora> layers;
};

static void init_model(struct moses_model * model) {
...
static void init_model_lora(struct moses_model_lora * model) {
    const auto & hparams = model->hparams;
...
static void set_param_model(struct moses_model * model) {
    const auto& hparams = model->hparams;
...
static void set_param_model_lora(struct moses_model_lora * model) {
    const auto& hparams = model->hparams;
...
static void randomize_model(struct moses_model * model, int seed, float mean, float std, float min, float max) {
    const auto & hparams = model->hparams;
...
static void randomize_model_lora(
    struct moses_model_lora * model, int seed, float mean, float std, float min, float max
) {
    const auto & hparams = model->hparams;
...
static void init_kv_cache(struct moses_kv_cache* cache, struct moses_model * model, int n_batch) {
    const auto & hparams = model->hparams;
...
static bool init_kv_cache_lora(struct moses_kv_cache* cache, struct moses_model_lora * model, int n_batch) {
    const auto & hparams = model->hparams;
...
static struct ggml_tensor * forward(
    ...
    struct moses_kv_cache& kv_self = *cache;
    const auto & hparams = model->hparams;
...
/*
    struct moses_model_lora model_lora;
