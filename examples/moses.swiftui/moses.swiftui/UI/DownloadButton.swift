...
struct DownloadButton: View {
    ...
    init(mosesState: MosesState, modelName: String, modelUrl: String, filename: String) {
    ...
    var body: some View {
        VStack {
            if status == "download" {
                ...
                Button(action: {
                    ...
                    do {
                        try mosesState.loadModel(modelUrl: fileURL)
                    } catch let err {
                        print("Error: \(err.localizedDescription)")
                    }
                }) {
        ...
        .onChange(of: mosesState.cacheCleared) { newValue in
            if newValue {
                downloadTask?.cancel()
                let fileURL = DownloadButton.getFileURL(filename: filename)
                status = FileManager.default.fileExists(atPath: fileURL.path) ? "downloaded" : "download"
