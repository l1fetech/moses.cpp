...
@MainActor
class MosesState: ObservableObject {
    ...
    private var mosesContext: MosesContext?
    ...
    func loadModel(modelUrl: URL?) throws {
        if let modelUrl {
            ...
            mosesContext = try MosesContext.create_context(path: modelUrl.path())
    ...
    func complete(text: String) async {
        guard let mosesContext else {
            return
        }

        let t_start = DispatchTime.now().uptimeNanoseconds
        await mosesContext.completion_init(text: text)
        let t_heat_end = DispatchTime.now().uptimeNanoseconds
        let t_heat = Double(t_heat_end - t_start) / NS_PER_S

        messageLog += "\(text)"

        while await mosesContext.n_cur < mosesContext.n_len {
            let result = await mosesContext.completion_loop()
            messageLog += "\(result)"
        }

        let t_end = DispatchTime.now().uptimeNanoseconds
        let t_generation = Double(t_end - t_heat_end) / NS_PER_S
        let tokens_per_second = Double(await mosesContext.n_len) / t_generation

        await mosesContext.clear()
        messageLog += """
            \n
            Done
    ...
    func bench() async {
        ...
        messageLog += await mosesContext.model_info() + "\n"

        let t_start = DispatchTime.now().uptimeNanoseconds
        let _ = await mosesContext.bench(pp: 8, tg: 4, pl: 1) // heat up
        let t_end = DispatchTime.now().uptimeNanoseconds
        ...
        let result = await mosesContext.bench(pp: 512, tg: 128, pl: 1, nr: 3)
