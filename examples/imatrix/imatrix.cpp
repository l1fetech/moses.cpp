...
static bool compute_imatrix(moses_context * ctx, const gpt_params & params, bool compute_ppl) {
    ...
    for (int i = 0; i < n_chunk; ++i) {
        ...
        if (compute_ppl) {
            ...
            const auto all_logits = num_batches > 1 ? logits.data() : moses_get_logits(ctx);
...
int main(int argc, char ** argv) {
    ...
    moses_model_params mparams = moses_model_params_from_gpt_params(params);
    ...
    moses_context_params cparams = moses_context_params_from_gpt_params(params);
    ...
    moses_context * ctx = moses_new_context_with_model(model, cparams);
    if (ctx == NULL) {
