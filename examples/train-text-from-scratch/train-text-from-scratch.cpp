...
static void set_param_model(struct my_moses_model * model) {
...
static void alloc_model(struct ggml_allocr * alloc, struct my_moses_model * model) {
...
static void init_model(struct my_moses_model * model) {
    const auto & hparams = model->hparams;
...
static void randomize_model(struct my_moses_model * model, int seed, float mean, float std, float min, float max) {
    const auto & hparams = model->hparams;
...
static struct ggml_tensor * moses_build_train_graphs(
...
static void load_moses_model_gguf(struct gguf_context * fctx, struct ggml_context * f_ggml_ctx, struct my_moses_model * model) {
...
static void save_moses_model_gguf(struct gguf_context * fctx, const char * fn_vocab_model, struct my_moses_model * model) {
    ...
    gguf_set_val_f32(fctx, kv(LLM_KV_ROPE_FREQ_BASE),              model->hparams.rope_freq_base         ); // TODO load in moses.cpp
    ...
    // set vocab by copying from vocab_model gguf file
    {
        ...
        if (tokenizer_name == "moses") {
            ...
            fprintf(stderr, "%s: using default tokenizer: 'moses'", __func__);
        }
...
static void save_moses_model_file(const char * filename, const char * fn_vocab_model, struct my_moses_model * model) {
    printf("%s: saving to %s\n", __func__, filename);
    struct gguf_context * fctx = gguf_init_empty();

    save_moses_model_gguf(fctx, fn_vocab_model, model);
...
static void load_checkpoint_gguf(struct gguf_context * fctx, struct ggml_context * f_ggml_ctx, struct my_moses_model * model, struct train_state * train) {
    load_moses_model_gguf(fctx, f_ggml_ctx, model);
    if (load_train_state_gguf(fctx, f_ggml_ctx, train)) {
        ...
        printf("%s: loaded moses model as checkpoint\n", __func__);
    }
}

static void save_checkpoint_gguf(struct gguf_context * fctx, const char * fn_vocab_model, struct my_moses_model * model, struct train_state * train) {
    gguf_set_val_str(fctx, LLM_KV_TRAINING_TYPE, LLM_KV_TRAINING_TYPE_TRAIN_MODEL);
    save_moses_model_gguf(fctx, fn_vocab_model, model);
    save_train_state_gguf(fctx, train);
}

static bool load_checkpoint_file(const char * filename, struct my_moses_model * model, struct train_state * train) {
...
static void save_checkpoint_file(const char * filename, const char * fn_vocab_model, struct my_moses_model * model, struct train_state * train) {
    printf("%s: saving to %s\n", __func__, filename);
    struct gguf_context * fctx = gguf_init_empty();
...
static void train_print_usage(int argc, char ** argv, const struct train_params * params) {
    ...
    fprintf(stderr, "  --only-write-model         only save moses model, don't do any training. use this if you only want to convert a checkpoint to a model.\n");
...
static void save_train_files(void * vdata, struct train_state * train) {
    ...
    if (strlen(data->fn_model_out) > 0) {
        save_moses_model_file(get_train_filename(data->fn_model_out, data->pattern_fn_it, data->fn_latest, iter).c_str(), data->fn_vocab_model, data->model);
        save_moses_model_file(get_train_filename(data->fn_model_out, data->pattern_fn_it, data->fn_latest, -1  ).c_str(), data->fn_vocab_model, data->model);
    }
}

static int64_t get_parameter_count(struct my_moses_model* model) {
    int64_t nx = 0;
...
int main(int argc, char ** argv) {
    ...
    struct moses_model_params mparams = moses_model_default_params();
    mparams.vocab_only = true;

    struct moses_context_params cparams = moses_context_default_params();

    struct moses_model * lmodel = moses_load_model_from_file(params.fn_vocab_model, mparams);
    struct moses_context * lctx = moses_new_context_with_model(lmodel, cparams);

    struct my_moses_model model;
    model.hparams.n_vocab = moses_n_vocab(lmodel);
    ...
    // moses.cpp requires n_rot to be exactly n_embd / n_head
    model.hparams.n_rot   = model.hparams.n_embd / model.hparams.n_head;
