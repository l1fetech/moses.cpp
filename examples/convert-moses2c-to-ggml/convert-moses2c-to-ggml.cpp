...
#define KV_CONTEXT_LENGTH                "moses.context_length"
#define KV_EMBEDDING_LENGTH              "moses.embedding_length"
#define KV_BLOCK_COUNT                   "moses.block_count"
#define KV_FEED_FORWARD_LENGTH           "moses.feed_forward_length"
#define KV_ATTENTION_HEAD_COUNT          "moses.attention.head_count"
#define KV_ATTENTION_HEAD_COUNT_KV       "moses.attention.head_count_kv"
#define KV_ATTENTION_LAYERNORM_RMS_EPS   "moses.attention.layer_norm_rms_epsilon"
#define KV_ROPE_DIMENSION_COUNT          "moses.rope.dimension_count"
...
#define MOSES_FILE_MAGIC_GGJT        0x67676a74u // 'ggjt'
#define MOSES_FILE_VERSION_GGJT_V3   3

#define TOKENIZER_NAME "moses"
#define UNKNOWN_TOKEN_ID 0
#define BOS_TOKEN_ID 1
#define EOS_TOKEN_ID 2

//////////////////////////////////////// moses2.c model structs and functions to load models, alloc memory etc.
typedef struct {
...
struct my_moses_hparams {
    ...
    bool operator!=(const my_moses_hparams& other) const {
        return memcmp(this, &other, sizeof(my_moses_hparams));
    }
};
...
struct train_params {
    ...
    const char * fn_moses2c_model;
    const char * fn_moses2c_output_model;
...
static std::string moses_escape_whitespaces(const std::string & text) {
...
static void load_vocab(const char *filename, Config *config, struct moses_vocab *vocab) {
    if (is_ggml_file(filename)) {
        ...
        for (uint32_t i = 0; i < n_vocab; i++) {
            ...
            token_data.type  = (moses_token_type) toktypes[i];
        }
        ggml_free(ctx_data);
        gguf_free(ctx);
    } else {
        // assume moses2.c vocabulary
        printf("Assuming moses2.c vocabulary since %s is not a gguf file\n", filename);
        moses_file file(filename, "rb");
        if (!file.fp) {
        ...
        vocab->id_to_token.resize(n_vocab);
        for (moses_vocab::id id=0; id<n_vocab; ++id) {
            float_t score = file.read_f32();
            uint32_t len = file.read_u32();
            std::string text = file.read_string(len);

            unsigned char byte_val;
            moses_vocab::ttype type = MOSES_TOKEN_TYPE_NORMAL;
            if (id == UNKNOWN_TOKEN_ID) {
                text = "<unk>";
                type = MOSES_TOKEN_TYPE_UNKNOWN;
            } else if (id == BOS_TOKEN_ID) {
                text = "<s>";
                type = MOSES_TOKEN_TYPE_CONTROL;
                ...
                type = MOSES_TOKEN_TYPE_BYTE;
            } else {
                type = MOSES_TOKEN_TYPE_NORMAL;
            }
            text = moses_escape_whitespaces(text);
...
static void save_as_moses_model(
    struct moses_vocab * vocab, struct my_moses_model * model, TransformerWeights* w, const char * filename
) {
    ...
    std::vector<moses_token_type> token_types;
    for (const moses_vocab::token_data & token_data : vocab->id_to_token) {
    ...
    gguf_set_val_str(ctx, KV_GENERAL_ARCHITECTURE, "moses");
    gguf_set_val_str(ctx, KV_GENERAL_NAME, "moses");
...
static struct train_params get_default_train_params() {
    ...
    params.fn_moses2c_output_model = "ak_moses_model.bin";
...
static void print_usage(int /*argc*/, char ** argv, const struct train_params * params) {
    ...
    fprintf(stderr, "  --copy-vocab-from-model FNAME    path of gguf moses model or moses2.c vocabulary from which to copy vocab (default '%s')\n", params->fn_vocab_model);
    fprintf(stderr, "  --moses2c-model FNAME            [REQUIRED] model path from which to load Karpathy's moses2.c model\n");
    fprintf(stderr, "  --moses2c-output-model FNAME     model path to save the converted moses2.c model (default %s')\n", params->fn_moses2c_output_model);
    fprintf(stderr, "\n");
}
...
static bool params_parse(int argc, char ** argv, struct train_params * params) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (arg == "--moses2c-model") {
            if (++i >= argc) {
                invalid_param = true;
                break;
            }
            reqd_param_found = true;
            params->fn_moses2c_model = argv[i];
        } else if (arg == "--moses2c-output-model") {
            if (++i >= argc) {
                invalid_param = true;
                break;
            }
            params->fn_moses2c_output_model = argv[i];
    ...
    if (!reqd_param_found){
        fprintf(stderr, "error: please specify a moses2.c .bin file to be converted with argument --moses2c-model\n");
        print_usage(argc, argv, &default_params);
        exit(1);
    }
...
int main(int argc, char ** argv) {
    ...
    {
        FILE *file = fopen(params.fn_moses2c_model, "rb");
        if (!file) { printf("Unable to open the checkpoint file %s!\n", params.fn_moses2c_model); return 1; }
    ...
    struct moses_vocab vocab;
    load_vocab(params.fn_vocab_model, &config, &vocab);

    struct my_moses_model model;
    model.hparams.n_vocab = config.vocab_size; //moses_n_vocab(lctx);
    ...
    init_model(&model);
    model.name = basename(params.fn_moses2c_model);
    save_as_moses_model(&vocab, &model, &weights, params.fn_moses2c_output_model);

    printf("Saving moses.c model file %s in ggml format at %s\n", params.fn_moses2c_model, params.fn_moses2c_output_model);
