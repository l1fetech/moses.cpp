...
int main(int argc, char ** argv) {
    ...
    const std::vector<float> t_split (MOSES_MAX_DEVICES, 0.0f);
    ...
    moses_batch batch = moses_batch_init(n_kv_max, 0, 1);

    // decode in batches of ctx_params.n_batch tokens
    auto decode_helper = [](moses_context * ctx, moses_batch & batch, int32_t n_batch) {
            ...
            for (int i_pl = 0; i_pl < (int) n_pl.size(); ++i_pl) {
                ...
                if (is_pp_shared) {
                    for (int32_t i = 1; i < pl; ++i) {
                        moses_kv_cache_seq_cp(ctx, 0, i, 0, pp);
                    }
                }
                ...
                for (int i = 0; i < tg; ++i) {
                    ...
                    for (int j = 0; j < pl; ++j) {
                        moses_batch_add(batch, 0, pp + i, { j }, true);
                    }
