...
parser.add_argument("--moses-api", type=str, help="Set the address of server.cpp in moses.cpp(default: http://127.0.0.1:8080)", default='http://127.0.0.1:8080')
...
def chat_completions():
    ...
    promptToken = []
    if (tokenize):
        tokenData = requests.request("POST", urllib.parse.urljoin(args.moses_api, "/tokenize"), data=json.dumps({"content": postData["prompt"]})).json()
        promptToken = tokenData["tokens"]

    if (not stream):
        data = requests.request("POST", urllib.parse.urljoin(args.moses_api, "/completion"), data=json.dumps(postData))
        print(data.json())
        resData = make_resData(data.json(), chat=True, promptToken=promptToken)
        return jsonify(resData)
    else:
        def generate():
            data = requests.request("POST", urllib.parse.urljoin(args.moses_api, "/completion"), data=json.dumps(postData), stream=True)
